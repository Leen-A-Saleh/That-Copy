...
if (!isset($_SESSION['user_id']) || ($_SESSION['user_role'] ?? '') !== 'THERAPIST') {
  header("Location: //That-Copy/Public/login/index.php");
  exit;
}
  ...
  try {
    ...
    if ($action === 'send_file') {
      ...
      echo json_encode([
        ...
        'sent_at'    => date('H:i'),
        'type'       => $type,
        'file_path'  => chat_public_file_url($path),
        'file_name'  => $origName,
      ]);
      exit;
    }
                ...
                <a href="?client=<?= $c['user_id'] ?>"
                  ...
                  <div class="chat-user-text">
                    ...
                    <p>
                      ...
                      <?php elseif ($c['last_type'] === 'VOICE'): ?>
                        <i class="fa-solid fa-microphone"></i> رسالة صوتية
                      <?php else: ?>
            ...
            <div class="chat-header">
              ...
              <div class="chat-actions">
                ...
                <button class="chat-action-btn call-btn" title="مكالمة صوتية">
                  <svg viewBox="0 0 24 24" fill="currentColor">
                    <path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56-.35-.12-.74-.03-1.01.24l-1.57 1.97c-2.83-1.49-5.15-3.8-6.62-6.63l1.97-1.57c.26-.27.36-.66.25-1.01-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3.3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .7-.75.7-1.2v-3.44c0-.54-.45-.98-.99-.98z" />
                  </svg>
                </button>
              ...
              <?php if (empty($messages)): ?>
                ...
                ?>
                  <div class="message <?= $mine ? 'me' : 'other' ?>">
                    <?php
                    $fileUrl = chat_public_file_url($m['file_path'] ?? null);
                    $msgType = strtoupper((string) ($m['type'] ?? 'TEXT'));
                    ?>
                    <?php if ($msgType === 'IMAGE' && $fileUrl !== ''): ?>
                      <a href="<?= htmlspecialchars($fileUrl) ?>" target="_blank" rel="noopener" class="msg-image-link">
                        <img src="<?= htmlspecialchars($fileUrl) ?>" alt="<?= htmlspecialchars((string) ($m['content'] ?? '')) ?>" class="msg-image">
                      </a>
                    <?php elseif ($msgType === 'FILE' && $fileUrl !== ''): ?>
                      <a href="<?= htmlspecialchars($fileUrl) ?>" target="_blank" rel="noopener" class="msg-file">
                        <i class="fa-solid fa-file"></i>
                        <span><?= htmlspecialchars((string) ($m['content'] ?? '')) ?></span>
                      </a>
                    <?php elseif ($msgType === 'VOICE' && $fileUrl !== ''): ?>
                      <audio controls preload="metadata" class="msg-voice" src="<?= htmlspecialchars($fileUrl) ?>"></audio>
                    <?php else: ?>
                      <?= nl2br(htmlspecialchars((string) ($m['content'] ?? ''))) ?>
                    <?php endif; ?>
